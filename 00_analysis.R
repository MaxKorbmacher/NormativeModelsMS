...
pacman::p_load(tidyverse,psych,effsize,ggseg,patchwork,rstatix,ggpubr,
               ...
               ggseg3d, ggsegExtra, mice, pwr, MASS)
# power
pwr.f2.test(u = 3, v = 43, f2 = NULL, sig.level = 0.05, power = .80)

# load data
cross = read.csv("/Users/max/Documents/Local/MS/NormativeModels/Regular/code/Zscores.csv")
long = read.csv("/Users/max/Documents/Local/MS/NormativeModels/Regular/code/Zscores_long.csv")
...
cross$nb_deviations = cross %>%
  dplyr::select(ends_with("z_score")) %>%
...
z_scores = cross %>% dplyr::select(ends_with("z_score"))
...
#
#Cortical plots
C1 = ggplot(dk %>% as_tibble() %>% left_join(z_cortical %>% dplyr::select(region,hemi,Z) %>% filter(group=="HC") %>% as_tibble())) + geom_brain(atlas = dk,aes(fill = Z),color="black")+
  scale_fill_gradient2(low = "blue",mid = "white",high="red",limits = c(-1.25,1.75)) +
  theme_void() + theme(legend.position="none")
C2 = ggplot(dk %>% as_tibble() %>% left_join(z_cortical %>% dplyr::select(region,hemi,Z) %>% filter(group=="MS") %>% as_tibble())) + geom_brain(atlas = dk,aes(fill = Z),color="black")+
  scale_fill_gradient2(low = "blue",mid = "white",high="red",limits = c(-1.25,1.75)) +
  theme_void() + theme(legend.position="none")
C3 = ggplot(dk %>% as_tibble() %>% left_join(z_cortical %>% dplyr::select(region,hemi,Z) %>% filter(group=="difference") %>% as_tibble())) + geom_brain(atlas = dk,aes(fill = Z),color="black")+
  scale_fill_gradient2(low = "blue",mid = "white",high="red",limits = c(-1.25,1.75)) +
  theme_void() + theme(legend.position="none")
...
metric <- "Accuracy"
# model <- train(diagnosis ~., data = cross%>%dplyr::select(ends_with("z_score"),diagnosis), 
#                method = "svmRadial", 
...
# #
# #
# model1 <- train(diagnosis ~., data = cross%>%dplyr::select(ends_with("volume"),diagnosis), 
#                method = "svmRadial", 
#                tuneLength = 50,preProc = c("center","scale"),  
#                metric=metric, trControl=control)
# model1
...
# model3 <- train(diagnosis ~., data = cross%>%dplyr::select(ends_with("z_score"),diagnosis), 
#                method = "svmRadial", 
#                tuneLength = 50,preProc = c("center","scale"),  
#                metric=metric)
# model3
...
# 
# 
# model4 <- train(diagnosis ~., data = cross%>%dplyr::select(ends_with("volume"),diagnosis), 
#                 method = "svmRadial", 
#                 tuneLength = 50,preProc = c("center","scale"),  
#                 metric=metric)
# model4
...
PASAT_OSL = PASAT_OSL %>% dplyr::select(eid,session,PASAT)
PASAT_OSL %>% dplyr::select(PASAT)%>% na.omit %>% nrow
...
fati_OSL = fati_OSL %>% dplyr::select(eid,session,fatigue)
...
fati$fatigue = fati %>% dplyr::select(A,     B,     C,     D,     E,     F,     G,     H,     I) %>% rowMeans()
fati = fati %>% dplyr::select(eid,session,fatigue)
...
run_and_plot <- function(long, predictor) {
  ...
  p1 <- ggplot(dk %>% as_tibble() %>%
                 left_join(test_cort %>% dplyr::select(region, hemi, std_coef), by = c("region", "hemi"))) +
...
run_and_extract <- function(df, predictor) {
  regions <- df %>% dplyr::select(ends_with("z_score")) %>% names()
...
# FSS
x = (lm(scale(rh_superiorfrontal_volume_z_score)~scale(fatigue),data = long2 %>% filter(session == 1)))
summary(x)
confint(x)
x = (lm(scale(lh_superiorfrontal_volume_z_score)~scale(fatigue),data = long2 %>% filter(session == 1)))
summary(x)
confint(x)
x = (lm(scale(rh_caudalmiddlefrontal_volume_z_score)~scale(fatigue),data = long2 %>% filter(session == 1)))
summary(x)
confint(x)
x = (lm(scale(rh_paracentral_volume_z_score)~scale(fatigue),data = long2 %>% filter(session == 1)))
summary(x)
confint(x)
x = (lm(scale(lh_supramarginal_volume_z_score)~scale(fatigue),data = long2 %>% filter(session == 1)))
summary(x)
confint(x)
...
# 3.1 Baseline ----
#
test = cross %>% filter(diagnosis == "MS") %>% dplyr::select(ends_with("z_score")) %>%
  mutate_all(~ ifelse((.) <= -1.96, 1, 0)) %>% 
  colSums() / nrow(cross %>% filter(diagnosis == "MS"))
test[order(data.frame(test)$test)]

max(cross %>% filter(diagnosis == "HC") %>% dplyr::select(ends_with("z_score")) %>%
  mutate_all(~ ifelse((.) <= -1.96, 1, 0)) %>% 
  colSums() / nrow(cross %>% filter(diagnosis == "MS")))
...
indi0 = ggplot(dk %>% as_tibble() %>% left_join(plot_indiv_c %>% dplyr::select(region,hemi,perc) %>% as_tibble())) + 
  geom_brain(atlas = dk,aes(fill = perc),color="red")+
  scale_fill_gradient2(low = "white",mid = "white",high="red",limits = c(0,27)) +
  theme_void() + theme(legend.position="none")
...
indi1 = ggplot(plot_indiv) + geom_brain(atlas = aseg, side = "coronal",aes(fill = perc),color="red")+
  scale_fill_gradient2(low = "white",mid = "white",high="red",limits = c(0,27)) + 
  #labs(title="Regional volume loss") + 
...
z_cols <- long %>% dplyr::select(ends_with("z_score")) %>% names()

# 2. Classify deviations: 1 if <= -1.96 else 0
long_dev <- long %>%
  dplyr::select(eid, age, all_of(z_cols)) %>%
...
binary_df <- long %>%
  dplyr::select(eid, age, session, ends_with("z_score")) %>%
...
compute_transitions <- function(data) {
  ...
  transition_list <- lapply(regions, function(region) {
    temp <- data %>%
      dplyr::select(eid, session, !!sym(region)) %>%
      arrange(eid, session) %>%
      group_by(eid) %>%
      mutate(
        prev = lag(!!sym(region)),
...
compute_dev_prop <- function(data) {
  data %>%
    dplyr::select(eid, session, ends_with("z_score")) %>%
...
plot_metric <- function(df, value_col,low,high, title = "") {
  ...
  p1 <- ggplot(dk %>% as_tibble() %>%
                 left_join(test_cort %>% dplyr::select(region, hemi, all_of(value_col)), by = c("region", "hemi"))) +
...
deviation_prop = deviation_prop %>% dplyr::select(label, mean_prop)
